...
export function newBusiness(name: string, currency: string, email: string): Business {
  return { id: crypto.randomUUID(), name, currency, suspended: false, demo: false, members: [{email, name: email.split('@')[0], role:'admin'}], categories: [...categories], expenses: [], ledger: [], audit: [] };
}
